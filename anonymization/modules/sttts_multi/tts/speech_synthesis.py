    ...
    def synthesize_speech(self, dataset_name, texts, speaker_embeddings, prosody=None, emb_level='spk'):
        ...
        if texts:
            ...
            if len(self.tts_models) == 1:
                instances = []
                for text, utt, speaker, language in texts:
                    try:
                        if emb_level == 'spk':
                        ...
                        if prosody:
                            ...
                            utt_prosody_dict = {}
                        instances.append((text, utt, speaker_embedding, utt_prosody_dict, language))
                    except KeyError:
                ...
                for iterator in text_iterators:
                    job_instances = []
                    for text, utt, speaker, language in iterator:
                        try:
                            if emb_level == 'spk':
                                speaker_embedding = speaker_embeddings.get_embedding_for_identifier(speaker)
                            else:
                            ...
                            if prosody:
                                ...
                                utt_prosody_dict = {}
                            job_instances.append((text, utt, speaker_embedding, utt_prosody_dict, language))
                        except KeyError:
                            logger.warn(f'Key error at {utt}')
                            continue
...
def synthesis_job(instances, tts_model, out_dir, sleep, text_is_phones=False, save_output=False):
    ...
    wavs = {}
    for text, utt, speaker_embedding, utt_prosody_dict, language in tqdm(instances):
        wav = tts_model.read_text(text=text, speaker_embedding=speaker_embedding, text_is_phones=text_is_phones,
                                  language=language, **utt_prosody_dict)
