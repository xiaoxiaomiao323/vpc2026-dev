...
class SpeakerExtraction:
    ...
    def extract_speakers(self, dataset_path, dataset_name=None, emb_level=None):
        ...
        if len(missing_utterances) > 0:
            ...
            spk2gender_path = dataset_path / 'spk2gender'
            if spk2gender_path.exists():
                spk2gender = read_kaldi_format(spk2gender_path)
            else:
                spk2gender = {spk: 'm' for spk in set(utt2spk.values())}
                logger.debug(f'spk2gender not found, using default for {len(spk2gender)} speakers')
    ...
    def _create_extractors(self, device):
        ...
        for single_vec_type in self.model_hparams['vec_type'].split('+'):
            if single_vec_type in {'xvector', 'ecapa','sslecapa'}:
