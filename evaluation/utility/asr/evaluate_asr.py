...
from .whisper_asr import InferenceWhisperASR
from .whisper_asr.inference import ASRDataset as ASRDataset_whisper
...
def evaluate_asr(eval_datasets, eval_data_dir, params, anon_data_suffix, device, backend):
    if backend == 'speechbrain':
        ...
        return asr_eval_whisper(eval_datasets=eval_datasets, eval_data_dir=eval_data_dir, params=params,
                                    anon_data_suffix=anon_data_suffix, device=device)
    else:
        raise ValueError(f'Unknown backend {backend} for ASR evaluation. Available backends: speechbrain.')

@torch.no_grad()
def asr_eval_whisper(eval_datasets, eval_data_dir, params, anon_data_suffix, device):
    model_type = params["model_type"]
    device = params["device"]

    model = InferenceWhisperASR( device=device)
    ...
    for test_set in test_sets:
        ...
        if (results_dir / test_set / 'wer').exists() and (results_dir / test_set / 'text').exists():
            ...
            dataset = ASRDataset_whisper(wav_scp_file=Path(data_path, 'wav.scp'), asr_model=model.asr_model)
